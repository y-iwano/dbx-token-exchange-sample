...
| 項目              | 設定値                                               |
| --------------- | ------------------------------------------------- |
| アプリケーション ID URI | `api://<Application (client) ID>`（自動生成）または任意の URI |
| スコープ名           | `access`（`REQUIRED_SCOPES` に設定する短縮形と一致させること）      |
| 同意できるユーザー       | 管理者とユーザー                                          |


> **App ID URI について:** MCP Inspector などが OAuth フローを実行する場合、Entra ID は `resource` パラメーターとスコープが同じ App ID URI を指すことを要求する。`BASE_URL/mcp` が `resource` として送信されるため、App ID URI を `BASE_URL/mcp`（例: `https://your-domain.com/mcp`）に設定し、`IDENTIFIER_URI` 環境変数に同じ値を設定することで整合性を保てる。

#### 1-2. トークンバージョンを v2.0 に設定（任意）
...
> これを設定しないと v1.0 トークン（issuer: `sts.windows.net`）が発行される。v1 トークンを使用する場合は、この設定を変更する代わりに `ENTRA_VERSION=1` を設定すること。
